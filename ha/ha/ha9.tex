...
\begin{itemize}
	...
	\begin{equation*}
		\psi(0) = \exists! y \ [\varphi(0, y)] = \exists! y \ [((0 = 0 \land y = 0) \lor 0 = S(x))] = \exists! y \ [(\top \land y = 0) \lor \bot] = \exists! y\ [y = 0]
	\end{equation*} 
	В силу уникальности $y = 0$ утверждение доказано.
	\item Индукционный переход. Допустим, мы знаем, что верно $\psi(x) = \exists! y \varphi(x, y)$. Покажем, что верно $\psi(S(x))$
	\begin{align*}
		\psi(S(x)) = &\exists! z \ [(\varphi(S(x), z))] = \exists! z \ [(S(x) = 0 \land z = 0) \lor (S(x) = S(z))]\\
