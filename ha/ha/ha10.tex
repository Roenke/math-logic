...
\begin{itemize}
	\item База: $\varphi(0) = \forall y \ 0 + y = 0 +' y$. Вместо этого достаточно показать, что $y = 0 +' y$. Это можем показать по индукции:
	\begin{itemize}
		...
		Доказано.
	\end{itemize}
	\item Покажем индукционный переход $S(x) + y = S(x) +' y$. Заметим, что $S(x) + y = S(x + y) = S(x +' y) = x +' S(y)$. Значит будет достаточно показать, что $x +' S(y) = S(x) +' y$. Сделаем это так же по индукции по $y$.
	\begin{itemize}
